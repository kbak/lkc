...
\begin{abstract}
This proposal describes a project that aims at improving usability of the Linux kernel configuration infrastructure. Current kernel tools have several serious problems, such as little support for validating kernel configurations, limited autoconfiguration to specific hardware, and no progressive disclosure for various types of users. We believe that improving these deficiencies will make the configuration process more reliable, automated, and easier to follow for non-experts. The document describes scope of the proposed project, evaluation plan, rough plan of action, and division of labor.
\end{abstract}
